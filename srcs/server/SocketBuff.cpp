...
bool SocketBuff::GetLine(std::string &line) {
    std::vector<char>::iterator it = std::find(buffer_.begin() + read_position_, buffer_.end(), '\n');
    if (it == buffer_.end()) {
        return false;
    }
...
ssize_t SocketBuff::FindString(const std::string &target) {
    std::vector<char>::iterator it = std::search(buffer_.begin() + read_position_, buffer_.end(), target.begin(), target.end());
    if (it == buffer_.end()) {
        return -1;
    }
...
bool SocketBuff::GetUntilCRLF(std::string &line) {
    const char CRLF[] = {'\r', '\n'};
    std::vector<char>::iterator it = std::search(buffer_.begin() + read_position_, buffer_.end(), CRLF, CRLF + 2);
    if (it == buffer_.end()) {
        return false;
    }
    line.assign(buffer_.begin() + read_position_, it);
