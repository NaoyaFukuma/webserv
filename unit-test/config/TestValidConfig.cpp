...
BOOST_AUTO_TEST_CASE(MultiPort) {
  ...
  BOOST_CHECK(servers[0].locations_[0].allow_methods_.find(GET) !=
              servers[0].locations_[0].allow_methods_.end());
  ...
  BOOST_CHECK(servers[1].locations_[0].allow_methods_.find(DELETE) !=
              servers[1].locations_[0].allow_methods_.end());
  BOOST_CHECK(servers[1].locations_[0].allow_methods_.find(GET) !=
              servers[1].locations_[0].allow_methods_.end());
  BOOST_CHECK(servers[1].locations_[0].allow_methods_.find(POST) !=
              servers[1].locations_[0].allow_methods_.end());
  ...
  BOOST_CHECK(servers[2].locations_[0].allow_methods_.find(GET) !=
              servers[2].locations_[0].allow_methods_.end());
  BOOST_CHECK(servers[2].locations_[0].allow_methods_.find(POST) !=
              servers[2].locations_[0].allow_methods_.end());
...
BOOST_AUTO_TEST_CASE(MultiRoute) {
  ...
  BOOST_CHECK_EQUAL(location2.allow_methods_.count(POST),
                    1); // POST method is allowed
...
BOOST_AUTO_TEST_CASE(MultiHost) {
  Config config;
  config.ParseConfig(
      MULTI_HOST); // Replace with actual path of your config file
  ...
  BOOST_CHECK_EQUAL(server1.is_default_server_,
                    true); // Default server is false
  BOOST_CHECK_EQUAL(server1.locations_[0].client_max_body_size_,
                    1024 * 1024); // Default max body size is 1MB
  BOOST_CHECK_EQUAL(server1.locations_[0].autoindex_,
                    false); // Default autoindex is false
  // BOOST_CHECK_EQUAL(server1.locations_[0].is_cgi_, false); // Default is_cgi
  // is false
  //  Add more default checks as necessary
  ...
  BOOST_CHECK_EQUAL(server2.is_default_server_,
                    false); // Default server is false
  BOOST_CHECK_EQUAL(server2.locations_[0].client_max_body_size_,
                    1024 * 1024); // Default max body size is 1MB
  BOOST_CHECK_EQUAL(server2.locations_[0].autoindex_,
                    false); // Default autoindex is false
  // BOOST_CHECK_EQUAL(server2.locations_[0].is_cgi_, false); // Default is_cgi
  // is false
  //  Add more default checks as necessary
}
...
BOOST_AUTO_TEST_CASE(Complex) {
  ...
  try {
    config.ParseConfig(
        COMPLEX_TEST); // Replace with actual path of your config file
  ...
  BOOST_CHECK_EQUAL(server3.locations_[0].client_max_body_size_,
                    10 * 1024 * 1024);
...
BOOST_AUTO_TEST_CASE(OneLine) {
  ...
  BOOST_TEST(server.listen_.sin_port ==
             htons(8080)); // Convert to network byte order
  ...
  BOOST_TEST(location.client_max_body_size_ ==
             1024 * 1024); // Default value 1MB
